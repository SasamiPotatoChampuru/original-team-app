    ...
    <div v-show="zero">
      ...
      <div class="ranking_flex"> 
        <div class="irasuto_ranking">
          <h1>イラスト</h1>
          <p>最高点</p>
          <button @click="check_irasuto" class="ranking_button">結果を見る！</button>
          <ul>
  ...
  export default{
    ...
    methods: {
      getDoc: async function() {
        const q = query(collection(db, "IrasutoUser"),orderBy("score","desc"),limit(100))
     ...
     getDoc_1: async function() {
        const q = query(collection(db, "IrasutoUser"),orderBy("score","desc"),limit(1))
        const querySnapshot = await getDocs(q);
        querySnapshot.forEach((doc) => {
        console.log(doc.id, "=>", doc.data());
        this.userData.push(doc.data())
     ...
     getDoc_quiz: async function() {
        const q = query(collection(db, "QuizUser"),orderBy("score","desc"),limit())
        const querySnapshot = await getDocs(q);
        querySnapshot.forEach((doc) => {
        console.log(doc.id, "=>", doc.data());
    ...
    getDoc_quiz_1: async function() {
        const q = query(collection(db, "QuizUser"),orderBy("score","desc"),limit(1))
        const querySnapshot = await getDocs(q);
        querySnapshot.forEach((doc) => {
        console.log(doc.id, "=>", doc.data());
        this.userData_2.push(doc.data())
...
<style>
  .design_ranking {
    width: 100vw;
    height: 100vh;
    background:linear-gradient(-130deg, #f5e941 20% , #fc9540 20% 50%, #f17007 50% 65%, rgb(5, 6, 104) 65%);
